...
function SpritePage (x,y,image) {
        ...
        this.dX = 0;
        this.dY = 0;
        this.widthMult = 1; //used to adjust the useWidth
        this.heightMult = 1; //used to adjust the useHeight
        ...
        this.jumpMax = 40;
        this.jumpCount = 0; // Sets the number of refresh cycles the character can jump - once set to zero (probably by some collision) -  this value is set to zero
        this.resize = function(mult){
            for (i = 0; i < this.frame.length; i++){
                for (k = 0; k < this.frame[i].length; k++){
                    this.frame[i][k].useWidth = this.frame[i][k].width*mult;
                    this.frame[i][k].useHeight = this.frame[i][k].height*mult;
                }
            }
        }
        this.jump = function(){
        ...
        this.draw = function() {
            if (this.visible) {
                ...
                ctx.drawImage(this.image,this.frame[this.currentRow][this.currentFrame].x,
                              this.frame[this.currentRow][this.currentFrame].y,this.frame[this.currentRow][this.currentFrame].width,this.frame[this.currentRow][this.currentFrame].height,this.x,this.y,this.frame[this.currentRow][this.currentFrame].useWidth,this.frame[this.currentRow][this.currentFrame].useHeight);
                ctx.globalAlpha = 1.0; //Setting the opacity back to 1 - which mean solid
